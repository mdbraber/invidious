...
private module Parsers
  ...
  module VideoRendererParser
    ...
    private def parse_internal(item_contents, author_fallback)
      ...
      # The same overlay marks a Short, carrying the literal text "SHORTS" where
      # a duration would be. The length branch above already special-cases it —
      # approximating 60s, because YouTube no longer reports a real duration for
      # Shorts — but then discards the *fact*, leaving clients to re-derive it
      # from that approximated length or from a "#shorts" title tag. Neither
      # works: a genuine 60-second upload looks identical, and the tag is just a
      # convention.
